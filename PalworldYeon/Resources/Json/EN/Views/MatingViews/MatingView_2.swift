...
struct MatingView_2: View {

    @StateObject private var enPalManager = EnPalManager()
    //@StateObject private var searchFieldViewModel = SearchFieldViewModel()

    //@StateObject private var matingViewModel = PalViewModel()

    @State private var selectedChildPalKey: String?
    ...
    var filteredPals: [EnPal] {
        enPalManager.enPals.filter {
    ...
    var body: some View {
        ...
        .onAppear {
            enPalManager.loadEnPalsData(){
                DispatchQueue.main.async {
                  }
              }
            enPalManager.loadBreedingData() {
